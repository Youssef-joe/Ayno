...
func (p *Processor) processEvent(event Event) error {
	...
	if p.analyticsSink != nil {
		p.analyticsSink.WriteEvent(context.Background(), event)
	}
...
func main() {
	analyticsSink, err := NewAnalyticsSinkFromEnv()
	if err != nil {
